...
from .iv import (
    ...
    implied_vol_from_price as _iv_from_price,
)
...
def build_gamma_df(
    ...
    *,
    ...
    """
    Returns columns: symbol, strike, expiration, gamma, theta, delta, type, iv, iv_source
      - iv_source in {"snapshot","inverted","default","shared"} where "shared" is per-expiry cache.
    Fallbacks compute missing greeks from BS when IV & T are available.
    """
    rows: List[dict] = []
    now_ts = now_ts or pd.Timestamp.utcnow()
    ...
    for sym, snap in (snaps or {}).items():
        ...
        if greeks is not None:
            ...
            delta = _clean_num(_get(greeks, "delta"))
        ...
        if iv_used is None and iv_top is not None:
            iv_used = iv_top
            iv_src = "snapshot"
        ...
        # Fallbacks
        if (gamma is None or theta is None or delta is None) and spot is not None:
            key = pd.to_datetime(exp).normalize()
            if iv_used is None and share_iv_across_exp and key in iv_by_exp:
                iv_used = iv_by_exp[key]; iv_src = "shared"
            if iv_used is None and default_iv is not None:
                iv_used = _clean_num(default_iv); iv_src = "default"
            ...
            if (T is not None) and (iv_used is not None):
                ...
                if delta is None:
                    delta = _bs_delta(spot, strike, T, risk_free_rate, iv_used, "call" if otype == "call" else "put")
                if share_iv_across_exp and iv_used is not None and iv_src in ("default","inverted"):
                    iv_by_exp[key] = iv_used

        if gamma is None and theta is None and delta is None:
            continue
    ...
    return pd.DataFrame(rows, columns=["symbol","strike","expiration","gamma","theta","delta","type","iv","iv_source"])

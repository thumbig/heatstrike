...
def build_pricing_df(
    ...
    *,
    ...
    """
    Returns columns:
      symbol,type,strike,expiration,market_price,theo_price,mispricing,mispricing_pct,
      iv_market,iv_market_source,iv_theo,iv_theo_source
    """
    now_ts = now_ts or pd.Timestamp.utcnow()
    rows: List[dict] = []
    ...
    for sym, snap in (snaps or {}).items():
        ...
        if greeks is not None:
            ...
            if iv_market is not None: iv_market_src = "snapshot"
        iv_top = _clean_num(_get(snap, "implied_volatility"))
        if iv_market is None and iv_top is not None:
            iv_market = iv_top; iv_market_src = "snapshot"
        if iv_market is None:
            iv_misc = _clean_num(_get(snap, "iv")) or _clean_num(_get(_get(snap, "metrics") or {}, "iv"))
            if iv_misc is not None:
                iv_market = iv_misc; iv_market_src = "snapshot"

        if iv_market is None:
            iv_market = _iv_from_price(price=mkt_px, S=spot, K=strike, T=T, r=risk_free_rate,
                                       opt_type=("call" if otype == "call" else "put"))
            if iv_market is not None: iv_market_src = "inverted"
        ...
        staged.append({
            ...
            "iv_market_source": iv_market_src,
        })
    ...
    for it in staged:
        ...
        if theo_iv_mode == "per_expiry_mean":
            ...
            iv_theo_source = "per_expiry_mean" if iv_theo is not None else ("default" if default_iv is not None else None)
            ...
            iv_theo_source = "snapshot_iv" if it["iv_market"] is not None else ("default" if default_iv is not None else None)
        elif theo_iv_mode == "default":
            iv_theo = float(default_iv) if default_iv is not None else None
            iv_theo_source = "default" if iv_theo is not None else None

        theo_price = None
        if iv_theo is not None and iv_theo > 0:
            theo_price = _bs_price(S=spot, K=it["strike"], T=it["T"], r=risk_free_rate,
        ...
        rows.append({
            "symbol": it["symbol"], "type": it["type"], "strike": it["strike"], "expiration": it["expiration"],
            "market_price": it["market_price"], "iv_market": it["iv_market"], "iv_market_source": it["iv_market_source"],
            "iv_theo": _clean_num(iv_theo), "iv_theo_source": iv_theo_source,
            "theo_price": _clean_num(theo_price), "mispricing": _clean_num(mispricing), "mispricing_pct": _clean_num(mispricing_pct),
        })

    return pd.DataFrame(rows, columns=[
        "symbol","type","strike","expiration",
        "market_price","iv_market","iv_market_source",
        "iv_theo","iv_theo_source","theo_price","mispricing","mispricing_pct"
    ])

...
from options_heatmap import (
    ...
    build_contracts_df,
    build_gamma_df,             # now includes theta + iv
)
...
OUTPUT_PATH = os.getenv("THETA_HEATMAP_PATH", "theta_heatmap.png")
DEFAULT_IV = os.getenv("DEFAULT_IV")
SHARE_IV_ACROSS_EXP = os.getenv("SHARE_IV_ACROSS_EXP", "1") not in ("0", "false", "False")
PER_EXPIRY_CAP = int(os.getenv("PER_EXPIRY_CAP", "10"))  # limit per expiry during fetch + selection
...
def main(trade: TradingClient | None = None,
    ...
    # Use your gvars/pvars auth layout, like in gamma app
    ...
    # >>> Page & balance contracts across expiries so we don't get 100 from a single one
    ...
    if gdf.empty:
        raise RuntimeError("No greeks available to plot (even after fallback).")

    # quick visibility: how many per expiry made it through
    try:
        by_exp = gdf.groupby(pd.to_datetime(gdf["expiration"]).dt.date).size()
        print(by_exp)
    except Exception:
        pass
    ...
    fig_or_ax = plot_theta_heatmap(
        gdf,
        title=f"Theta Heatmap for {UNDERLYING}",
        save_path=OUTPUT_PATH,
        side_by_side=True,
        annotate="symbol",       # or "symbol", "iv" or None
        annotate_fmt=".1f",
    )
    if fig_or_ax:
        print(f"Saved theta heatmap to: {os.path.abspath(OUTPUT_PATH)}")
